...
select ft_pt, GEN, sum(firsttime_ug) as FT_UG, sum(other_firsttime) as OTH_FT_UG, sum(other_ug) as OTH_UG, sum(nondeg_ug) as ND_UG,
       sum(firsttime_gr) as FT_GR, sum(other_gr) as OTH_GR, sum(nondeg_gr) AS ND_GR, count(id) as Count
    FROM CDS_B WITH (NOLOCK)
group by ft_pt, GEN
order BY
    case
    when ft_pt = 'FT' and GEN = 'Men'  then 1
    when ft_pt = 'FT' and GEN = 'Women'  then 2
    when ft_pt = 'FT' and GEN = 'Another Gender' then 3
    when ft_pt = 'PT' and GEN = 'Men'  then 4
    when ft_pt = 'PT' and GEN = 'Women' then 5
    when ft_pt = 'PT' and GEN = 'Another Gender'  then 6 END