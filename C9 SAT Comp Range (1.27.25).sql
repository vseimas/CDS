...
---       sum(case when SACOMP_Range  = '1400-1600' then 1 else 0 END) As SATC_TOTAL,
       format(cast(sum(case when SACOMP_Range = '1400-1600' then 1 else 0 END) *100.00 / count(case when Test_Score in ('SAT','Both') then PIDM_KEY END) as decimal(10,5)), '0.00') AS SATC_PERC
...
----       count(PIDM_KEY) AS EnrStu,
---       count(case when Test_Score in ('SAT','Both') then pidm_key END) AS SATTestStu,
---       sum(case when SACOMP_Range  = '1200-1399' then 1 else 0 END) As SATC_TOTAL,
       format(cast(sum(case when SACOMP_Range = '1200-1399' then 1 else 0 END) *100.00 / count(case when Test_Score in ('SAT','Both') then PIDM_KEY END) as decimal(10,5)), '0.00') AS SATC_PERC
...
----       count(PIDM_KEY) AS EnrStu,
---       count(case when Test_Score in ('SAT','Both') then pidm_key END) AS SATTestStu,
---       sum(case when SACOMP_Range  = '1000-1199' then 1 else 0 END) As SATC_TOTAL,
       format(cast(sum(case when SACOMP_Range = '1000-1199' then 1 else 0 END) *100.00 / count(case when Test_Score in ('SAT','Both') then PIDM_KEY END) as decimal(10,5)), '0.00') AS SATC_PERC
...
----       count(PIDM_KEY) AS EnrStu,
---       count(case when Test_Score in ('SAT','Both') then pidm_key END) AS SATTestStu,
---       sum(case when SACOMP_Range  = '800-999' then 1 else 0 END) As SATC_TOTAL,
       format(cast(sum(case when SACOMP_Range = '800-999' then 1 else 0 END) *100.00 / count(case when Test_Score in ('SAT','Both') then PIDM_KEY END) as decimal(10,5)), '0.00') AS SATC_PERC
...
----       count(PIDM_KEY) AS EnrStu,
---       count(case when Test_Score in ('SAT','Both') then pidm_key END) AS SATTestStu,
---       sum(case when SACOMP_Range  = '600-799' then 1 else 0 END) As SATC_TOTAL,
       format(cast(sum(case when SACOMP_Range = '600-799' then 1 else 0 END) *100.00 / count(case when Test_Score in ('SAT','Both') then PIDM_KEY END) as decimal(10,5)), '0.00') AS SATC_PERC

    from SCR_RANGES WITH (NOLOCK)

        UNION ALL

Select '400-599'  as SATRNG,
----       count(PIDM_KEY) AS EnrStu,
---       count(case when Test_Score in ('SAT','Both') then pidm_key END) AS SATTestStu,
---       sum(case when SACOMP_Range  = '400-599' then 1 else 0 END) As SATC_TOTAL,
       format(cast(sum(case when SACOMP_Range = '400-599' then 1 else 0 END) *100.00 / count(case when Test_Score in ('SAT','Both') then PIDM_KEY END) as decimal(10,5)), '0.00') AS SATC_PERC
